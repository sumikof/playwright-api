...
9. **plan 整合性チェック(実装前ゲート、双方向)**:
   ...
   いずれかを満たさない、または plan が spec に無い設計判断を追加・承認済み設計から逸脱する場合は、
...
13. **最終レビュー指摘の解消(受け入れの前提)**: 最新レビューの全指摘について、Claude が
    妥当性を検証し、次のいずれかで決着させる — (a) 修正して最新 SHA で再レビュー、または
    (b) 非対応の根拠を PR に記録し、ユーザーが明示的に受け入れる。修正を行えば設計側と同じく
    その時点で従前の最終レビューは**無効化**され、再レビュー(手順12)へ戻る。未決着の指摘が
    残る限り手順14へ進まない
...
- いずれの段階でも、レビュー後に内容が変わったらそのレビューは無効化し、最新 SHA で再レビューする。
  また最新レビューの全指摘は「修正して再レビュー」か「非対応の根拠記録 + ユーザー明示受け入れ」で
