...
| 自動ゲート(CI/保護) | **当面導入しない**。人間ゲート + ローカル検証 + 簡易チェックリストで担保。**CI 導入を「推奨する次ステップ」**として明記(整合の厳密保証を移管) |
...
- 整合の確認(簡易・手動)の原則が記載されている(承認・受け入れ・マージは最後にレビューした内容に
  対して行う / 変更後は再レビュー / マージ前に base 最新を取り込む)、かつ**厳密・原子的な保証は CI
  の責務**として次ステップに委ねる旨が明記されている
...
- **既知の制限(整合の手動運用)**: 承認/レビュー/マージ対象の一致や base 追随の確認は、CI・
  ブランチ保護が無いため**人間の簡易チェックリスト**で担保する(§7「整合の確認」)。厳密な SHA 一致や、
  人間確認〜マージ実行間の TOCTOU 窓は手動運用では原理的に完全には塞げない。**ソロ・単一 PR・
  低頻度運用では実害が小さい**ため当面は残余リスクとして受容する。
- **推奨する次ステップ: CI 導入**。上記の厳密・原子的な保証は本来 CI の責務(必須ステータスチェック +
  branch-up-to-date 保護 + auto-merge / merge queue)。手動ゲートを厚くするより CI へ移管するのが
  正攻法であり、本方針では CI 導入を次の正式ステップとして推奨する(§8)。導入時期・トリガーは
  体制やリリース頻度の変化に応じて判断する。
