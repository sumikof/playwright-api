...
15. ユーザーがマージ(直前に `Closes #<N>` へ変更)。**マージ時 HEAD = 受け入れ対象 SHA =
    Codex 最終レビュー済み SHA の三者一致**に加え、**base ブランチが最終レビュー時の base SHA から
    動いていないこと**を必須とする(動いていれば新 base を取り込んで再検証+最終レビュー)。
    CI・ブランチ保護が無いため、これらは人間が手動で確認する。→ Issue close、残課題は別 Issue へ
...
- **最終マージ**: マージ時 HEAD = ユーザー受け入れ SHA = Codex 最終レビュー済み SHA(三者一致)。
  加えて **base SHA の一致**も必須 — 最終レビュー時の base ブランチ SHA を PR に記録し、マージ直前に
  base が動いていないことを確認する。base が別 PR 等で進んでいたら、その新 base を取り込んで
  (rebase/merge)ローカル検証と最終レビューをやり直す(統合結果が未検証・未レビューのままマージ
  されるのを防ぐ。CI・「branch up to date 必須」保護が無いための手動ゲート)
...
- SHA 一致の不変条件が記載されている(承認時 HEAD = 承認 SHA = 設計レビュー済み SHA の三者一致、
  マージ時 HEAD = 受け入れ SHA = 最終レビュー済み SHA の三者一致、かつマージ時 base = 最終レビュー時
  base SHA)
