...
11. ブランチを base に追随させ(`git pull` / merge で最新 base を取り込む)、ローカル検証
    (`npm test`, `npx playwright test` 等)を実行して結果を PR に記録し、Draft 解除する
...
- **承認・受け入れ・マージは「最後にレビューした内容」に対して行う**。レビュー後に内容を変えたら
  再レビューする(設計フェーズは承認まで、実装フェーズは受け入れまでが対象)
- **マージ前に base の最新を取り込む**(`git pull`/merge)。レビュー・検証は最新 base 込みの状態で行う
...
- **CI 導入時に移管する保証**(次ステップの具体像):
  ...
  - `auto-merge` / merge queue により「最新 base 込みでレビュー済みの内容」を原子的にマージ
