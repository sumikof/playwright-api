...
6. **承認前に HEAD を進める変更があれば最新 SHA で `@codex review` を再実行**する(設計レビューの
   再取得)。spec の変更に限らず、README や運用文書など spec 以外のコミットで HEAD が進んだ場合も
   同様に再レビューし、「最後に Codex が設計レビューした SHA」が最新 HEAD に一致するまで承認へ
   進まない(手順7の三者一致を回復する手段)
...
12. `@codex review`(最終レビュー用テンプレ)。最終レビュー対象 SHA(= HEAD)を PR に記録する
    (手順11 により、この HEAD は現在の base を祖先として包含している)
...
15. ユーザーがマージ(直前に `Closes #<N>` へ変更)。マージ直前に次の2条件を人間が確認する:
    - **HEAD = 受け入れ対象 SHA = Codex 最終レビュー済み SHA**(三者一致)
    - **現在の base が HEAD の祖先**(`git merge-base --is-ancestor <base> HEAD`。HEAD が最新 base を
      包含している)
    両方が成り立てばマージ結果は「レビュー済み HEAD そのもの」になり、未検証・未レビューの統合は
    混入しない。base が HEAD に未包含なら base を取り込み(rebase/merge)→ HEAD が変わるため
    再検証+最終レビューをやり直す。→ Issue close、残課題は別 Issue へ

### SHA 一致の不変条件(自動ゲートが無い運用の要)

- **設計承認**: 承認時 HEAD = 承認 SHA = 最後に Codex が設計レビューした SHA(三者一致)
- **最終マージ**: マージ時 (1) HEAD = ユーザー受け入れ SHA = Codex 最終レビュー済み SHA の三者一致、
  かつ (2) 現在の base が HEAD の祖先(HEAD が最新 base を包含)。この2条件でマージ結果はレビュー済み
  HEAD と一致し、未検証・未レビューの統合が混入しない。base が HEAD に未包含なら取り込み → 再検証 +
  再レビュー。base SHA を別途記録・比較する簿記は不要(祖先性チェックが同じ保証を与える)。
  ※ 人間による確認からマージ実行までの微小な TOCTOU 窓は手動運用では完全には塞げず、
  **将来 CI の branch-up-to-date 保護 + auto-merge / merge queue** で閉じる(§14 既知の制限)
- **無効化の対象期間を区別する**(常に最新 HEAD で再レビューを要求するわけではない):
  - **設計レビュー**は「承認までに HEAD を進める変更」で無効化する(spec 変更に限らず、承認前の
    非 spec コミットも含む。手順6 で再レビューし三者一致を回復する)。一方、承認後の plan 作成や
    通常の実装コミットでは設計レビューを無効化しない(それらは §7 手順9 と「設計レビューへ戻す条件」に
    従い、重大な設計変更のときだけ設計レビューへ戻す)
  - **最終レビュー**は「受け入れまでの **PR の全追跡対象ファイル**の変更」で無効化する
    (spec・コード・テストに限らず、`docs/operations/ai-workflow-policy.md`・`CLAUDE.md`・
    `AGENTS.md`・README・plan・PR テンプレート等を含む。最終レビュー後に HEAD を進める変更は
    すべて再検証+再レビュー対象)
...
- SHA 一致の不変条件が記載されている(承認時 HEAD = 承認 SHA = 設計レビュー済み SHA の三者一致、
  マージ時 HEAD = 受け入れ SHA = 最終レビュー済み SHA の三者一致、かつマージ時に現在の base が
  HEAD の祖先(HEAD が最新 base を包含))
...
- **既知の制限(SHA/base 整合の手動運用)**: 本設計の SHA 三者一致・base 祖先性の確認は、CI・
  ブランチ保護が無いため**人間の手動確認**で担保する。人間の確認からマージ実行までの微小な
  TOCTOU 窓(確認後にマージ寸前で base が進む等)は手動運用では原理的に完全には塞げない。
  これは**将来 CI 化(branch-up-to-date 保護 + auto-merge / merge queue、必須ステータスチェック)で
  自動・原子的に閉じる**。それまでは残余リスクとして受容する(ソロ・低頻度運用では実害が小さい)。
- **未決**: 将来 CI を導入する時期・トリガー(体制やリリース頻度の変化で再検討)。上記の手動 SHA/base
  ゲートを自動化する動機になる。
